...
function parseResponse(response: any) {
  ...
  const snapshotSection = sections.get("Snapshot") ?? sections.get("Page Snapshot");
  let pageState = sections.get("Page state") ?? sections.get("Page");
  if (pageState && snapshotSection) {
    const snapshotText = snapshotSection.includes("```") ? snapshotSection : `\`\`\`yaml\n${snapshotSection}\n\`\`\``;
    if (!pageState.includes("Page Snapshot"))
      pageState = `${pageState}\n- Page Snapshot:\n${snapshotText}`;
  }
